...
export default function MovimientosPage() {
  ...
  useEffect(() => {
    api<Movement[]>('/operations/movements')
      .then(setMovimientos)
      .catch((cause) =>
    ...
    () =>
      movimientos.filter(
        (item) =>
          ...
          `${item.referencia} ${item.operacion} ${item.comprobante} ${item.tercero} ${item.origen} ${item.destino}`
            .toLowerCase()
    ...
    <div className="module-page kardex-page">
      <div className="module-head">
        <div className="module-title">
          ...
          <span>{movimientos.length} movimientos trazables</span>
        </div>
      </div>
      ...
      <div className="module-tools">
        <label className="pill-search">
          ...
          <input
            ...
            placeholder="Buscar comprobante, tercero, referencia o almacén"
          />
        </label>
        <select
              ...
              visibles.map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span
                        className={`status ${item.tipo === 'ENTRADA' ? 'status-green' : 'status-blue'}`}
                      >
              ...
              <tr>
                <td colSpan={7}>
                  <div className="table-empty">
                    <Search size={22} />
                    <span>No hay movimientos que coincidan con la búsqueda.</span>
                  </div>
                </td>
              </tr>
            )}
        ...
        <div className="modal-backdrop">
          ...
          >
            <div className="modal-top">
              <div>
                <h2 id="kardex-detail-title">{seleccionado.referencia}</h2>
                <small>
                  {seleccionado.operacion} · {seleccionado.comprobante} ·{' '}
                  {new Date(seleccionado.fecha).toLocaleString('es-PE')}
                </small>
              </div>
              <button
            ...
            <div className="operation-detail kardex-detail">
              ...
              <div className="kardex-summary-grid">
                <div>
                  <FileText size={17} />
                  <span>
                    Documento<strong>{seleccionado.comprobante}</strong>
                    <small>{seleccionado.tercero}</small>
                  </span>
                </div>
                <div>
                  <Boxes size={17} />
                  <span>
                    Movimiento<strong>{seleccionado.tipo}</strong>
                    <small>{quantity(seleccionado.unidades)} unidades</small>
                  </span>
                </div>
                <div>
                  <UserRound size={17} />
                  <span>
                    Responsable<strong>{seleccionado.responsable}</strong>
                    <small>{seleccionado.estado}</small>
                  </span>
                </div>
              </div>
              <div className="kardex-route">
                <div>
                  <small>Sale de</small>
                  <strong>{seleccionado.origen}</strong>
                </div>
                <ArrowRight size={20} />
                <div>
                  <small>Llega a</small>
                  <strong>{seleccionado.destino}</strong>
                </div>
              </div>
              ...
              <div className="kardex-products-heading">
                ...
                <div>
                  ...
                  <small>El cálculo muestra el saldo antes y después de cada movimiento.</small>
                </div>
              </div>
              <div className="kardex-product-list">
                {seleccionado.detalles.map((item, index) => {
                    ...
                    <article className="kardex-product-card" key={`${item.producto}-${index}`}>
                      <div className="kardex-product-head">
                        ...
                        <span className={`status ${entry ? 'status-green' : 'status-blue'}`}>
                          {entry ? '+' : '−'}
                          {quantity(item.cantidad)} unidades
                        </span>
                      </div>
              ...
              <div className="modal-actions">
                <button className="btn-secondary" onClick={() => setSeleccionado(null)}>
                  Cerrar
                </button>
              </div>
            </div>
