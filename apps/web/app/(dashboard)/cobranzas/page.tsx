...
export default function CollectionsPage() {
  ...
  async function load() {
    ...
    setForm((current) => ({
      ...current,
      saleId: current.saleId || debtData[0]?.id || '',
      amount: current.amount || Number(debtData[0]?.debtAmount ?? 0),
    }));
  }
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Clientes con deuda, pagos parciales e historial de cobros.
        </p>
      </div>
      ...
      <form onSubmit={submit} className="panel grid gap-3 p-4 md:grid-cols-4">
          ...
          >
            ...
            {debts.map((sale) => (
              <option key={sale.id} value={sale.id}>
                {sale.client?.name} - {sale.ticketNumber} - {money(sale.debtAmount)}
              </option>
            ))}
          </select>
        </label>
        ...
        <label>
          ...
          <input
            ...
            min="0.01"
            step="0.01"
            max={Number(selected?.debtAmount ?? 0)}
            value={form.amount}
        ...
        {message ? (
          <p className="rounded-md bg-emerald-50 p-2 text-sm font-semibold text-emerald-700 md:col-span-4">
            {message}
          </p>
        ) : null}
        {error ? (
          <p className="rounded-md bg-rose-50 p-2 text-sm font-semibold text-rose-700 md:col-span-4">
            {error}
          </p>
        ) : null}
      </form>
