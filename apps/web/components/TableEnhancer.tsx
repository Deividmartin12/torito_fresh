...
export function TableEnhancer() {
  useEffect(() => {
    const cleanups: (() => void)[] = [];
    const enhance = () => {
      document.querySelectorAll<HTMLTableElement>(".glass-table table, .table-wrap table, table.table").forEach((table) => {
        ...
        const hasPageSearch = Boolean(pageHost?.querySelector(".module-tools .pill-search, .operations-filters .pill-search, .report-filters .pill-search"));
        ...
        const render = () => {
          ...
          let previous = 0;
          const pageButtons = numbers.map((value) => {
            const ellipsis = previous && value - previous > 1 ? `<span class="pagination-ellipsis" aria-hidden="true">…</span>` : "";
            previous = value;
          ...
          pagination.querySelector<HTMLSelectElement>("select")?.addEventListener("change", (event) => {
            pageSize = Number((event.currentTarget as HTMLSelectElement).value);
            page = 1;
            render();
          });
          pagination.querySelectorAll<HTMLButtonElement>("button[data-page]").forEach((button) => button.addEventListener("click", () => {
            page = Number(button.dataset.page);
            render();
          }));
        };
        input?.addEventListener("input", () => { query = input.value.toLowerCase().trim(); page = 1; render(); });
        render();
        cleanups.push(() => { toolbar?.remove(); pagination.remove(); delete table.dataset.enhanced; });
      });
    };
    enhance();
