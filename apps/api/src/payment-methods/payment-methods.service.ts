...
export class PaymentMethodsService {
  ...
  async update(id: string, dto: UpdatePaymentMethodDto) {
    await this.find(id);
    try {
      return this.view(await this.prisma.metodoPago.update({ where: { id: BigInt(id) }, data: this.updateData(dto) }));
    } catch (error) {
      this.handlePrismaError(error);
    }
  }
  ...
  private async find(id: string) {
    try {
      ...
      if (row) return row;
    } catch { /* Invalid identifiers are treated as missing records. */ }
  ...
  private view(row: { id: bigint; nombre: string; requiereOperacion: boolean; estado: boolean }) {
    return { id: row.id.toString(), nombre: row.nombre, requiereOperacion: row.requiereOperacion, estado: row.estado };
  }
