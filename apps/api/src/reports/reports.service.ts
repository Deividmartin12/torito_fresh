...
export class ReportsService {
  ...
  async business(from?: string, to?: string) {
    ...
    const [sales, expenses, stocks, firstSales] = await Promise.all([
      this.prisma.venta.findMany({
        ...
        include: {
          ...
          movimientosInventario: { where: { tipoOperacion: "VENTA", estado: "CONFIRMADO" }, include: { detalles: true } },
          cuentaCobrar: { include: { pagos: { where: { estado: "CONFIRMADO" }, include: { metodoPago: true } } } },
        },
      }),
    ...
    type Period = { key: string; label: string; sales: number; expenses: number; cost: number; margin: number; orders: number };
    type Ranking = { id: string; name: string; value: number; count: number };
    type ProductRanking = { id: string; name: string; quantity: number; revenue: number; cost: number; margin: number };
    ...
    const heatmap = new Map<string, { day: number; dayLabel: string; hour: number; orders: number; sales: number }>();
    const firstSaleByClient = new Map(firstSales.map((row) => [row.clienteId.toString(), row._min.fecha?.getTime()]));
    ...
    for (const sale of sales) {
      ...
      for (const detail of sale.detalles) {
        const returnedQuantity = detail.detallesDevolucion.reduce((sum, item) => sum + Number(item.cantidad), 0);
        const netQuantity = Math.max(Number(detail.cantidad) - returnedQuantity, 0);
        const movement = movementDetails.find((item) => item.productoId === detail.productoId && item.loteId === detail.loteId);
        const unitCost = Number(movement?.costoUnitario ?? detail.producto.costoReferencia);
        const lineCost = netQuantity * unitCost;
        const share = Number(sale.subtotal) > 0 ? Number(detail.subtotal) / Number(sale.subtotal) : 0;
        const lineRevenue = netBase * share;
        saleCost += lineCost;
        const current = products.get(detail.productoId.toString()) ?? { id: detail.productoId.toString(), name: detail.producto.nombre, quantity: 0, revenue: 0, cost: 0, margin: 0 };
      ...
      totalCost += saleCost;
      this.addPeriod(days, this.dayKey(sale.fecha), this.dayLabel(sale.fecha), { sales: netSale, cost: saleCost, margin: netBase - saleCost, orders: 1 });
      this.addPeriod(months, this.monthKey(sale.fecha), this.monthLabel(sale.fecha), { sales: netSale, cost: saleCost, margin: netBase - saleCost, orders: 1 });
      this.addRanking(zones, sale.cliente.direccion?.trim() || "Sin zona registrada", sale.clienteId.toString(), netSale);
      ...
      const heat = heatmap.get(heatKey) ?? { day: local.weekday, dayLabel: local.weekdayLabel, hour: local.hour, orders: 0, sales: 0 };
      heat.orders += 1; heat.sales += netSale; heatmap.set(heatKey, heat);
      for (const payment of sale.cuentaCobrar?.pagos ?? []) {
        paymentMethods.set(payment.metodoPago.nombre, (paymentMethods.get(payment.metodoPago.nombre) ?? 0) + Number(payment.monto));
      }
    }
    ...
    for (const expense of expenses) {
      ...
      this.addPeriod(days, this.dayKey(expense.fecha), this.dayLabel(expense.fecha), { expenses: amount });
      this.addPeriod(months, this.monthKey(expense.fecha), this.monthLabel(expense.fecha), { expenses: amount });
      this.addRanking(expenseCategories, expense.categoria, expense.categoria, amount);
    }

    const lowStockByProduct = new Map<string, { id: string; name: string; available: number; minimum: number }>();
    for (const product of stocks) {
      const key = product.id.toString();
      lowStockByProduct.set(key, {
        id: key,
        name: product.nombre,
        available: product.stocks.reduce((sum, stock) => sum + Math.max(Number(stock.cantidad) - Number(stock.cantidadReservada), 0), 0),
    ...
    return {
      ...
      expenseCategories: [...expenseCategories.values()].sort((a, b) => b.value - a.value).slice(0, 10),
      customerMix,
      paymentMethods: [...paymentMethods].map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value),
      heatmap: [...heatmap.values()],
      lowStock: [...lowStockByProduct.values()].filter((row) => row.available <= row.minimum).sort((a, b) => a.available - b.available).slice(0, 10),
    };
  }

  private dateRange(from?: string, to?: string) {
    const start = from ? new Date(`${from}T00:00:00-05:00`) : new Date(new Date().getFullYear(), new Date().getMonth() - 11, 1);
  ...
  private addPeriod(target: Map<string, any>, key: string, label: string, values: Partial<{ sales: number; expenses: number; cost: number; margin: number; orders: number }>) {
    const row = target.get(key) ?? { key, label, sales: 0, expenses: 0, cost: 0, margin: 0, orders: 0 };
    for (const [field, value] of Object.entries(values)) row[field] += value ?? 0;
    target.set(key, row);
  }

  private addRanking(target: Map<string, { id: string; name: string; value: number; count: number }>, name: string, id: string, value: number) {
    const row = target.get(name) ?? { id, name, value: 0, count: 0 };
    row.value += value; row.count += 1; target.set(name, row);
  }

  private dayKey(date: Date) { return new Intl.DateTimeFormat("en-CA", { timeZone: "America/Lima", year: "numeric", month: "2-digit", day: "2-digit" }).format(date); }
  private monthKey(date: Date) { return this.dayKey(date).slice(0, 7); }
  private dayLabel(date: Date) { return new Intl.DateTimeFormat("es-PE", { timeZone: "America/Lima", day: "2-digit", month: "short" }).format(date); }
  private monthLabel(date: Date) { return new Intl.DateTimeFormat("es-PE", { timeZone: "America/Lima", month: "short", year: "2-digit" }).format(date); }
  private localParts(date: Date) {
    const parts = new Intl.DateTimeFormat("en-US", { timeZone: "America/Lima", weekday: "short", hour: "2-digit", hourCycle: "h23" }).formatToParts(date);
    const weekdayName = parts.find((part) => part.type === "weekday")?.value ?? "Mon";
    const weekdays: Record<string, [number, string]> = { Mon: [0, "Lunes"], Tue: [1, "Martes"], Wed: [2, "Miercoles"], Thu: [3, "Jueves"], Fri: [4, "Viernes"], Sat: [5, "Sabado"], Sun: [6, "Domingo"] };
    const [weekday, weekdayLabel] = weekdays[weekdayName] ?? [0, "Lunes"];
    return { weekday, weekdayLabel, hour: Number(parts.find((part) => part.type === "hour")?.value ?? 0) };
  }
