...
async function main() {
  ...
  const tipoProductoPorNombre = Object.fromEntries(tiposProducto.map((tipo) => [tipo.nombre, tipo]));
  ...
  await prisma.trabajador.upsert({
    ...
    update: { nombres: "Administrador", apellidos: "Torito Fresh", cargo: "Administrador", estado: true },
    create: {
      tipoDocumento: "DNI",
  ...
  await Promise.all(
    [
      { ruc: "20600111221", razonSocial: "Aguas del Norte S.A.C.", nombreComercial: "Aguas del Norte" },
      { ruc: "20600111222", razonSocial: "Envases Peruanos E.I.R.L.", nombreComercial: "Envases Peruanos" },
      { ruc: "20600111223", razonSocial: "Suministros Lima S.A.", nombreComercial: "Suministros Lima" },
    ].map((proveedor) => prisma.proveedor.upsert({
      where: { ruc: proveedor.ruc }, update: { ...proveedor, estado: true }, create: proveedor,
    })),
  );

  await Promise.all(
    [
      { tipoDocumento: "DNI", numeroDocumento: "45871236", nombreLegal: "Juan Perez", telefono: "987654321", direccion: "Av. Los Olivos 123", limiteCredito: 300 },
      { tipoDocumento: "RUC", numeroDocumento: "20600123456", nombreLegal: "Restaurante El Buen Sabor", telefono: "955222111", direccion: "Jr. Comercio 450", limiteCredito: 1500 },
      { tipoDocumento: "RUC", numeroDocumento: "10456789123", nombreLegal: "Minimarket La Esquina", telefono: "944333222", direccion: "Calle Central 890", limiteCredito: 800 },
    ].map((cliente) => prisma.cliente.upsert({
      where: { numeroDocumento: cliente.numeroDocumento }, update: { ...cliente, estado: true }, create: cliente,
    })),
  );
  ...
  const [almacenPrincipal, disponible, productosOperativos] = await Promise.all([
    ...
    prisma.producto.findMany({ where: { codigo: { in: ["AGUA-20L", "BIDON-20L", "DISP-MANUAL"] } } }),
  ]);

  for (const producto of productosOperativos) {
    const stock = await prisma.stockAlmacen.findFirst({
      where: { productoId: producto.id, almacenId: almacenPrincipal.id, loteId: null, estadoInventarioId: disponible.id },
    });
    if (!stock) {
      await prisma.stockAlmacen.create({
        data: {
          ...
          cantidad: producto.codigo === "AGUA-20L" ? 100 : producto.codigo === "BIDON-20L" ? 40 : 24,
